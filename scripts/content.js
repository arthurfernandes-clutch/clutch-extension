window.addEventListener("load", () => {
    const setIframe = (id = "new-ext-iframe", style = {}, classes = [], htmlContent, darkMode = false) => {
    ...
    * {
    ...
    const setOpenCloseEvent = (iframeId) => {
        ...
        iframe.contentWindow.document.querySelector(".close__button").addEventListener("click", () => {
            ...
            iframe.setAttribute('style', iframe.getAttribute('style') + `width: 50px; height: 50px; border-radius: 100%; min-height: inherit;`);
        })
    }
    ...
    const getOptions = () => {
        ...
        (options) => {
            ...
            if (options.extension) {
                ...
                let freshdesk = options.freshdesk ? `<a href="https://withclutch.freshdesk.com/a/tickets/filters/544307" class="add-continer__button" id="tickets">Tickets</a>`: "";
                let taktile = options.taktile ? `<a href="https://app.taktile.com/" class="add-continer__button" id="taktile">Taktile</a>` : "";

                let linksToadd = ``;
                options.linksList.forEach((l) => {
                    linksToadd += `<a href="${l.url}" class="add-continer__link">${l.title}</a>`;
                });
